...
export async function queryOmnicode(prompt: string): Promise<string> {
  ...
  try {
    ...
    clearTimeout(timer);
    if (!res.ok) throw new Error(`OmniCode HTTP ${res.status}`);
    ...
    clearTimeout(timer);
    if (err instanceof Error && err.name === "AbortError") return "OmniCode timed out.";
    throw err;
  }
}
